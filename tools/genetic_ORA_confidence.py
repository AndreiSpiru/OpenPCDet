...
def evaluate_and_save(individual, datasets, original_points, attack_paths, max_length, args, cfg):
    """
    Evaluate the fitness of an individual by applying the attack and calculating the detection IOU.
    ...
    scores, _ = validation.detection_confidence_custom_dataset(args, cfg, attack_paths)
    validation_utils.create_or_modify_excel_generic(scores, attack_paths, args.ckpt, type="genetic_confidence", file_path="genetic_ORA_confidence_results.xlsx")
    logging.critical(f"Mean score: {np.mean(scores)}")
    print(f"Mean score: {np.mean(scores)}")
    return (np.mean(scores),)
