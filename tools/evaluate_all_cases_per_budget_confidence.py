...
# python evaluate_all_cases_per_budget_copnfidence.py --cfg_file cfgs/kitti_models/pointpillar.yaml --ckpt pointpillar_7728.pth --data_path ~/mavs_code/output_data_converted/0-10 --result_path slbz.xlsx
            ...
            if os.path.isdir(sensor_path):
                for condition in os.listdir(sensor_path):
                    ...
                    if os.path.isdir(condition_path):
                        ...
                        confidences, _ = validation.detection_confidence_custom_dataset(case_args, cfg, attack_paths)

                        for threshold in thresholds:
                            # Create or modify the Excel file with the evaluation results
                            utils.create_or_modify_excel_recall_confidence(case_args.result_path, case_args.data_path, threshold, case_args.ckpt, confidences, budget, "random")
