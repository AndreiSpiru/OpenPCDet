...
def main():
    """
    Main function to calculate the mean confidence of detections in the dataset.
    """
    ...
    for condition in os.listdir(root_path):
        ...
        if os.path.isdir(condition_path):
            ...
            # Calculate confidence scores and get the validation file list
            confidence, validation_file_list = validation.detection_confidence(case_args, cfg)
            confidences.append(confidence)

    # Flatten the list of confidence scores and calculate the mean
    flattened_confidences = np.array(flatten(confidences))
    mean_confidence = np.mean(flattened_confidences)
